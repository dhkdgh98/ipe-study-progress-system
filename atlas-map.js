(function(global){
  ...
  function createAtlasMapRuntime(api){
    ...
    function buildGraph(baseConcepts){
      const state=api.getState(),perf=api.getPerformance();
      const baseIds=new Set(baseConcepts.map(c=>c.id));
      const conceptIds=new Set(baseIds);
      for(const concept of baseConcepts){
        for(const id of concept.parents||[])conceptIds.add(id);
        for(const child of perf.children(concept.id))conceptIds.add(child.id);
        for(const id of concept.related||[])conceptIds.add(id);
      }
      const concepts=[...conceptIds].map(perf.conceptById).filter(Boolean);
      const visibleIds=new Set(concepts.map(c=>c.id));
      const nodes=[],edges=[],keywords=new Map();
      for(const concept of concepts){
        ...
        for(const [section] of api.sections)for(const line of concept.sections?.[section]||[])for(const word of line.keywords||[]){
          if(!word)continue;if(!keywords.has(word))keywords.set(word,new Set());keywords.get(word).add(concept.id);
        }
      }
      for(const [word,owners] of keywords){
        const id='kw:'+word;nodes.push({id,kind:'keyword',title:word,radius:15,childCount:0});
        for(const owner of owners)edges.push({from:owner,to:id,type:'keyword'});
      }
      return {nodes,edges,baseIds};
    }

    function layout(graph){
      const conceptNodes=graph.nodes.filter(n=>n.kind==='concept'),keywordNodes=graph.nodes.filter(n=>n.kind==='keyword');
      const byId=new Map(graph.nodes.map(n=>[n.id,n])),depth=new Map();
      ...
      function getDepth(id,seen=new Set()){
        if(depth.has(id))return depth.get(id);if(seen.has(id))return 0;seen.add(id);
        const value=parents.has(id)?1+Math.min(...parents.get(id).map(parent=>getDepth(parent,new Set(seen)))):0;depth.set(id,value);return value;
      }
      const levels=new Map();
      for(const node of conceptNodes){const d=getDepth(node.id);if(!levels.has(d))levels.set(d,[]);levels.get(d).push(node)}
      const gapX=150,gapY=145;
      for(const [level,items] of levels){
        items.sort((a,b)=>b.childCount-a.childCount||a.title.localeCompare(b.title));
        const columns=Math.max(1,Math.ceil(Math.sqrt(items.length*2)));
        items.forEach((node,index)=>{node.x=(index%columns)*gapX;node.y=level*gapY+Math.floor(index/columns)*95});
      }
      const maxConceptX=Math.max(0,...conceptNodes.map(n=>n.x));
      const kwColumns=Math.max(1,Math.ceil(Math.sqrt(keywordNodes.length)));
      keywordNodes.forEach((node,index)=>{node.x=maxConceptX+230+(index%kwColumns)*125;node.y=(index/kwColumns|0)*70});
      const minX=Math.min(0,...graph.nodes.map(n=>n.x)),minY=Math.min(0,...graph.nodes.map(n=>n.y));
      for(const node of graph.nodes){node.x-=minX-80;node.y-=minY-80}
      return byId;
    }

    function html(baseConcepts){
      const id='atlas-canvas-'+(++serial),graph=buildGraph(baseConcepts);layout(graph);
      requestAnimationFrame(()=>mount(id,graph));
    ...
    function mount(id,graph){
      ...
      const canvas=shell.querySelector('canvas'),ctx=canvas.getContext('2d',{alpha:false});
      ...
      function draw(){
        if(destroyed)return;const rect=canvas.getBoundingClientRect();ctx.setTransform(dpr,0,0,dpr,0,0);ctx.fillStyle='#07101c';ctx.fillRect(0,0,rect.width,rect.height);ctx.save();ctx.translate(view.x,view.y);ctx.scale(view.scale,view.scale);
        ctx.lineCap='round';
        for(const edge of graph.edges){const a=byId.get(edge.from),b=byId.get(edge.to);if(!a||!b||(!visible(a,rect)&&!visible(b,rect)))continue;ctx.beginPath();ctx.moveTo(a.x,a.y);if(edge.type==='related'){const mx=(a.x+b.x)/2+18,my=(a.y+b.y)/2-18;ctx.quadraticCurveTo(mx,my,b.x,b.y)}else ctx.lineTo(b.x,b.y);ctx.strokeStyle=COLORS[edge.type];ctx.globalAlpha=edge.type==='keyword'?.2:.58;ctx.lineWidth=(edge.type==='hierarchy'?2.2:1.6)/view.scale;ctx.setLineDash(edge.type==='related'?[8/view.scale,7/view.scale]:edge.type==='keyword'?[2/view.scale,7/view.scale]:[]);ctx.stroke()}
        ctx.setLineDash([]);ctx.globalAlpha=1;
        for(const node of graph.nodes){if(!visible(node,rect))continue;const r=node.radius;ctx.beginPath();ctx.arc(node.x,node.y,r,0,Math.PI*2);ctx.fillStyle=node.kind==='keyword'?'#102d2b':'#10223b';ctx.fill();ctx.strokeStyle=node.kind==='keyword'?COLORS.keyword:COLORS[node.role]||COLORS.general;ctx.lineWidth=(node.base?4:2)/view.scale;ctx.stroke();if(view.scale>.32){ctx.fillStyle='#eef7ff';ctx.font=`${Math.max(10,12/view.scale)}px system-ui`;ctx.textAlign='center';ctx.textBaseline='middle';const max=node.kind==='keyword'?13:16;const label=node.title.length>max?node.title.slice(0,max-1)+'…':node.title;ctx.fillText((node.kind==='keyword'?'#':'')+label,node.x,node.y-(node.kind==='concept'&&view.scale>.55?5:0));if(node.kind==='concept'&&view.scale>.55){ctx.fillStyle='#a9bbd2';ctx.font=`${Math.max(8,9/view.scale)}px system-ui`;ctx.fillText(`하위 ${node.childCount}`,node.x,node.y+13)}}}
        ctx.restore();
      }
      ...
      shell.querySelector('[data-map-fit]').onclick=fit;
      shell.querySelector('[data-map-fullscreen]').onclick=async()=>{if(document.fullscreenElement)await document.exitFullscreen();else await shell.requestFullscreen();setTimeout(fit,80)};
      const observer=new ResizeObserver(()=>resize());observer.observe(canvas);resize();fit();
      active={destroy(){destroyed=true;observer.disconnect();if(raf)cancelAnimationFrame(raf)}};
    }
